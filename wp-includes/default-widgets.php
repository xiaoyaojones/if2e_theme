...
?>
			...
			<li><a href="<?php bloginfo('rss2_url'); ?>" title="<?php echo esc_attr(__('Syndicate this site using RSS 2.0')); ?>"><?php _e('Entries <abbr title="Really Simple Syndication">RSS</abbr>'); ?> <i class="ico-rss"></i></a></li>
			<li><a href="<?php bloginfo('comments_rss2_url'); ?>" title="<?php echo esc_attr(__('The latest comments to all posts in RSS')); ?>"><?php _e('Comments <abbr title="Really Simple Syndication">RSS</abbr>'); ?> <i class="ico-rss"></i></a></li>
<?php
			/**
	...
	function widget( $args, $instance ) {
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {
				$output .=  '<li class="recentcomments">' . get_comment_author_link() . ': <a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '" title="' . get_the_title($comment->comment_post_ID) . '">' . mb_strimwidth(strip_tags($comment->comment_content), 0, 80,"...") . '</a>' . '</li>';
			}
		}
		$output .= '</ul>';
		$output .= $after_widget;
